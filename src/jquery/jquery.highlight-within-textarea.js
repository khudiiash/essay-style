...
(function($) {
	...
	HighlightWithinTextarea.prototype = {
		...
		getStringRanges: function(input, str) {
			...
			let index = 0;
			while (index = inputLower.indexOf(strLower, index), index !== -1 && (!isLetter(input[index-1]) && !isLetter(inputLower[index + strLower.length]))) {
		...
		renderMarks: function(boundaries) {
			...
			input = input.replace(/<mark><mark>/g,'<mark>')
			input = input.replace(/<\/mark><\/mark>/g,'</mark>')
