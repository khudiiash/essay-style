...
export default function checkPrepositional(text, suggestions) {
    ...
    let matches = [],
        ...
        prep2 = /([a-zA-Z]+\s?\b)\s\bof\b\s([a-zA-Z]+\s?\b){1,3}\s\bof\b\s([a-zA-Z]+\s?\b){1,2}/gi,
        // the effect of the influence of corporation on state politics
        prep3 = /([a-zA-Z]+\s?\b){1,2}\s\bof\b\s([a-zA-Z]+\s?\b){1,3}\s\bon\b\s([a-zA-Z]+\s?\b){1,3}\s\bof\b\s([a-zA-Z]+\s?\b){1,2}/gi
